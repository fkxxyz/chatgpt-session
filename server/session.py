...
@app.route('/api/session/params', methods=['POST'])
def handle_params():
    id_ = flask.request.args.get('id')
    session, r = get_session_query(id_)
    if r is not None:
...
@app.route('/api/session/create', methods=['PUT'])
def handle_create():
    id_ = flask.request.args.get('id')
...
@app.route('/api/session/inherit', methods=['PUT'])
def handle_inherit():
    id_ = flask.request.args.get('id')
    if id_ is None or len(id_) == 0:
        return flask.make_response('error: missing id query', http.HTTPStatus.BAD_REQUEST)
...
@app.route('/api/session/reload', methods=['PATCH'])
def handle_reload():
    id_ = flask.request.args.get('id')
    session, r = get_session_query(id_)
    if r is not None:
...
@app.route('/api/session/get', methods=['GET', 'PATCH'])
def handle_get():
    id_ = flask.request.args.get('id')
    session, r = get_session_query(id_)
    if r is not None:
...
@app.route('/api/session/status_all')
...
@app.route('/api/session/remark', methods=['GET', 'PUT'])
def handle_remark():
    id_ = flask.request.args.get('id')
    session, r = get_session_query(id_)
    if r is not None:
...
@app.route('/api/session/compress', methods=['PATCH'])
def handle_compress():
    id_ = flask.request.args.get('id')
    session, r = get_session_query(id_)
    if r is not None:
...
@app.route('/api/session/once', methods=['POST'])
def handle_send_once():
    level = flask.request.args.get('level', type=int, default=65536)
    message_info = flask.request.get_json()
    msg_str = message_info.get('message')
    if msg_str is None or len(msg_str) == 0:
        return flask.make_response('error: missing message', http.HTTPStatus.BAD_REQUEST)
    try:
        reply = globalObject.scheduler.send_away(msg_str, level)
    except error.ChatGPTSessionError as e:
        return flask.make_response(str(e), e.HttpStatus)
    return reply
